...
- [x] The website reports success.
- [x] An `Inquiries` tab exists.
- [x] The inquiry appears in that tab.
- [x] `notification_status` says `Sent`.
- [x] The Contact alert reaches `pipathmath@gmail.com`.
...
The live Payment Link URL is runtime configuration and is not hardcoded into the deployed website. Replace it only in Cloudflare's Production `STRIPE_PAYMENT_LINK_URL_AUGUST_2026` variable; do not put the live URL in `wrangler.jsonc`, `.dev.vars.example`, or source code. The tracked local Wrangler configuration does not contain a Payment Link. For local payment testing, put only a sandbox link in the ignored `.dev.vars` file.
